package xyz.bluspring.cblegacy.render.cache;
...
public class InMemoryQuadCompressor implements Runnable
{
	...
	private static CacheMap<float[][], WeakReference<float[][]>> cachelvl2 = new CacheMap<float[][], WeakReference<float[][]>>( new CacheMap.EqTest() {
	...
	private static CacheMap<float[], WeakReference<float[]>> cache = new CacheMap<float[], WeakReference<float[]>>( new CacheMap.EqTest() {
